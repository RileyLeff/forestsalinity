setwd("/Users/kgedan/Documents/projects_ongoing/VCR/forest disturbance/forest disturbance 2023")
library("dplyr")
library("tidyverse")
library("lubridate")
...
soil = read.csv("soil salinity 2023_Noa.csv")
str(soil)
  soil$date = mdy(soil$date)
  soil$replicate = as.factor(soil$replicate)
  
pore = read.csv("Porewater_Salinity_Brownsville_2023.csv")
str(pore)
  pore$Date = mdy(pore$Date)
...
#should use soil ppt data, since we have a more full record and it is likely to be more consistent through time 
#soil ppt should be less affected by precipitation or drought than porewater ppt concentrations
#though porewater concentrations are more likely to reflect physiological stress of trees

arrange(ppt_plot, soil_ppt)
arrange(ppt_plot, pore_ppt)

#Compare 2022 and 2023
soil2022 = read.csv("soil salinity 2022_Aliya.csv", stringsAsFactors = T)
soil2022 = soil2022 %>% mutate(Salinity_ppt = 0.49*EC_ms.cm) 
ppt_plot2022 = soil2022 %>% group_by(plot) %>% 
...
ppt22 = soil2022 %>% select(plot, Salinity_ppt) %>% group_by(plot) %>% summarise(mean = mean(Salinity_ppt, na.rm=T), se = se(Salinity_ppt)) %>%
  mutate(year = 2022)
ppt23 = soil %>% select(plot, Salinity_ppt) %>% group_by(plot) %>% summarise(mean = mean(Salinity_ppt, na.rm=T), se = se(Salinity_ppt)) %>%
  mutate(year = 2023)
...
ppt22_23mean = ppt22_23 %>% group_by(level, year) %>% summarise( se = se(mean), mean = mean(mean, na.rm=T))
...
BVLppt = read.csv("Forest Disturbance Plot_Porewater Salinity_2019_2023.csv", stringsAsFactors = T)
...
ggplot(BVLppt, aes(x = ppt, color=level))+ geom_density()

BVLppt$plot = factor(BVLppt$plot, levels = c("H0", "H1", "H2", "H3", "H4", "H5", "H6", "H7",
...
BVLppt_mean = BVLppt%>% select(plot, date_collected, ppt, level) %>% group_by(plot, level, date_collected) %>% summarise(mean = mean(ppt)) %>%
  group_by(level, date_collected) %>% summarise(se = se(mean), mean = mean(mean, na.rm = T))
...
##### TREES #####
data<-read.csv(file = "trees_2023.csv", sep=",", stringsAsFactors = T)
...
d1 = trunk1 %>% group_by(Year) %>% select(Year, Tree.Code, alive.1.or.dead.0) %>% 
...
mortality = d1 %>% pivot_longer(cols = c(died2020, died2021, died2022, died2023),names_to = "year", names_prefix = "died", values_to = "died") %>% 
  select(-c(alive2021, alive2019, alive2020, alive2022, alive2023))


by_species_level_year_plot =  d1 %>% pivot_longer(cols = c( alive2019, alive2020, alive2021, alive2022, alive2023),names_to = "year", names_prefix = "alive", values_to = "alive") %>% 
  group_by(Species, Level, year, plot) %>% select(-c(died2020, died2021, died2022, died2023))
...
by_species_plot_prop_dead = dead_live %>% group_by(Species, plot) %>% summarise(prop_dead = mean(prop_dead))
by_species_plot_prop_dead %>% group_by(Species) %>% count() # how many plots have records for each species? 
...
prop_dead_ppt = prop_dead_ppt %>% filter(Species != "NA") %>% filter(Species != "Uni") %>%
  filter(Species != "Car") %>% filter(Species != "Cor") %>% filter(Species != "Vac") %>%
  filter(Species != "Que") %>% filter(Species != "Liq")
...
all_species_prop_dead = dead_live %>% group_by(plot) %>% summarise(
  nlive = sum(nlive), 
  ndied = sum(ndied), 
...
prop_dead_lt_ppt = prop_dead_lt_ppt %>% filter(Species != "NA") %>% filter(Species != "Uni") %>%
  filter(Species != "Car") %>% filter(Species != "Cor") %>% filter(Species != "Vac") %>% 
  filter(Species != "Que") %>% filter(Species != "Liq")
...
ppt22_23_mean = ppt22_23 %>% group_by(plot) %>% summarise(ppt_mean = mean(mean, na.rm=T))
...
#filter to species that are present in >3 levels
prop_dead_soil_mean = prop_dead_soil_mean %>% filter(Species != "NA") %>% filter(Species != "Uni") %>%
  filter(Species != "Car") %>% filter(Species != "Cor") %>% filter(Species != "Vac") %>% 
  filter(Species != "Que") %>% filter(Species != "Liq")
...
temp = mortality_noR %>% filter(year == 2022) %>% arrange(plot, Species)
